...
const boirFields = [
  ...
  },
  {
    ...
    fieldCategory: 'reporting_company',
    isRequired: true,
    options: JSON.stringify(["Initial report", "Correction", "Update", "Newly exempt entity"]),
  ...
  },
  {
    ...
    fieldCategory: 'beneficial_owner',
    isRequired: true,
    options: JSON.stringify(["U.S. passport", "State drivers license", "State/local/tribal government ID", "Foreign passport"]),
...
async function addBoirFieldsDirect() {
  try {
    console.log('Clearing existing BOIR fields for service ID 11...');
    
    // Delete existing fields for service 11
    await pool.query('DELETE FROM service_custom_fields WHERE service_id = 11');
    console.log('Cleared existing fields');

    console.log(`Adding ${boirFields.length} comprehensive BOIR Filing fields...`);
    
    let successCount = 0;
    for (const field of boirFields) {
      try {
        const query = `
    ...
    console.log(`\nSuccessfully added ${successCount}/${boirFields.length} BOIR Filing fields`);
    ...
    console.log(`Total: ${successCount} comprehensive BOIR fields`);
