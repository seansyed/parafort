...
async function finalBoirSelectFix() {
  try {
    console.log('Adding the final 7 BOIR select fields with correct PostgreSQL array format...');
    ...
    for (const field of selectFields) {
      try {
        ...
        // Insert with proper PostgreSQL array syntax
        const query = `
        ...
        await pool.query(query, [
          ...
          null, // placeholder
          field.helpText,
          field.options, // Pass array directly for text[] column
          field.displayOrder,
          field.fieldGroup,
          true // is_active
        ]);
    ...
    if (successCount === selectFields.length) {
      ...
      console.log('\nBOIR Filing service is now ready for comprehensive customer intake!');
    }
